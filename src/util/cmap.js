...
export default class CMap {
        ...
        switch (i) {
          case "font":
            if (!styleOption._font) {
              ...
              styleOption.font = styleOption[i].replace(
                strArr[0],
                size * this.DPI + unit
              );
            }
      ...
      for (let i = 0, l = _point.size; i < l; i++) {
        ...
        if (/-|C/g.test(_obj.data.toString())) {
          do {
            ...
            y = parseFloat(
              (_obj.y[0] + _obj.height * Math.random()).toFixed(2)
            );
  ...
  drawPointPop(_point, ctx) {
    ...
    if (!_point.popAnimate) {
      ...
      for (let i = 0, l = _point.popAnimate.r.length; i < l; i++) {
        _point.popAnimate.r[i] = popAnimateFun(
          _point.popAnimate.r[i],
          _point.popAnimate.RSpeed
        );
      }
    }
  }
  ...
  drawMessage(_point, ctx) {
    ...
    if (!_point.lineLength) {
      _point.lineLength = parseFloat(
        Math.sqrt(Math.pow(_point.x, 2) + Math.pow(_point.y, 2)).toFixed(2)
      );
      ...
      let speedRandom = Math.random() + 0.02;
      let _x = _point.x;
  ...
  cityName() {
    ...
    for (let i = 0, l = this.areas.length; i < l; i++) {
      ...
      if (this.inAreaCtx == i) {
        ...
        ctx.fillText(
          _.name,
          _.xCenter + translateX + (_.move ? _.move[0] : 0),
          _.yCenter + translateY + (_.move ? _.move[1] : 0)
        );
      } else {
        ...
        ctx.fillText(
          name,
          _.xCenter + translateX + translateX + (_.move ? _.move[0] : 0),
          _.yCenter + translateY + (_.move ? _.move[1] : 0)
        );
      }
  ...
  cityPoint(point, ctx, pointSet) {
    if (pointSet.pop) this.drawPointPop(point, ctx);
  ...
  lazyAnimate() {
    ...
    if (
      this.Mirror.canvas.cityArea.isPointInPath(this.currentX, this.currentY)
    ) {
      this.onMap = 1;
    } else {
      this.inAreaCtx = -1;
    ...
    this.ele.addEventListener("mousemove", function(event) {
      ...
      if (_self.onMap > -1) {
        ...
        if (_self.callback && _self.callback.mousemove)
          _self.callback.mousemove(
            _self.inAreaCtx,
            _self.areas[_self.inAreaCtx],
            event
          );
      }
      // 在地图外
  ...
  autoSize() {
    ...
    let dataClear = mapSizeInfo => {
      ...
      for (let c = 0, d = this.options.city.data.length; c < d; c++) {
        this.options.city.data[c].map = this.claerMultiPolygon(
          this.options.city.data[c].map
        );
    ...
    // 对数据进行清 MultiPolygon
    this.options.cityArea.data = this.claerMultiPolygon(
      this.options.cityArea.data
    );

    mapSizeInfo = this.computedData(this.options.cityArea.data);
    this.minScale = Math.min(
      (this.ctxW - cityArealineW) / mapSizeInfo.width,
      (this.ctxH - cityArealineW) / mapSizeInfo.height
    );
  ...
  setArea() {
    ...
    let Area = function(index, obj, computedData, cityInfo) {
      ...
      this.warn = {}; // 保存错误信息
      this.origin = obj;
      this.show = obj.show === false ? false : true;
      this.move = obj.move || null;
    };
    ...
    for (let i = 0, l = this.options.city.data.length; i < l; i++) {
      ...
      if (_data.map) {
        // 计算宽高
        _computedData = this.computedData(_data.map, _data.fixCenter);
      } else {
  ...
  createCanvas() {
    let create = () => {
      ...
      let boxW = parseFloat(
        this.ele.style.width || window.getComputedStyle(this.ele, null).width
      );
      let boxH = parseFloat(
        this.ele.style.height || window.getComputedStyle(this.ele, null).height
      );
    ...
    // 如果有点 再添加一个
    if (
      this.options.city.point &&
      this.options.city.point.size &&
      (this.options.city.point.pop || this.options.message)
    ) {
      create();
    }
  }
  ...
  clear() {
    let canvas = this.ele.getElementsByTagName("canvas")[0];
    canvas && this.ele.removeChild(canvas);
  }
}
